...
public class EjerciciosVector3_5 : MonoBehaviour
{
    [SerializeField]
    ...
    void Update()
    {
        if (VectorA.x > VectorB.x && VectorA.z > VectorB.z) 
        {
            VectorC.x -= Time.deltaTime * speed;
            VectorC.z -= Time.deltaTime * speed;

            if(VectorC.x < VectorB.x && VectorC.z < VectorB.z) 
            {
                VectorC = VectorA;
            }
        }
